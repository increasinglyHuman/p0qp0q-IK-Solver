...
export class OctahedralBoneHelper extends Group {
	...
	constructor( mesh, options = {} ) {
		...
		this.options = {
			...
			// CRITICAL: Actual geometry measurements from Blender model
			octahedronTipDistance: options.octahedronTipDistance || 2.30045, // Distance from origin to tip
			sphereTemplateRadius: options.sphereTemplateRadius || 0.0255,    // Template sphere radius
	...
	_createJointSphere( bone ) {
		...
		// This should now align perfectly with the octahedron tip!
		...
		tailSphere.position.copy( childBone.position ); // At bone tail (= octahedron tip position)
		bone.add( tailSphere );

		// Return head sphere for tracking
		return headSphere;
